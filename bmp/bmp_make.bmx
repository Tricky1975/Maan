...
Function PackResources()
	...
	Next
	If projectini.list("Import")
		For Local l$=EachIn projectini.list("Import")
			Local original$
			Local target$
			Local p = l.find(">")
			If p>-1
			...
			Else
				original = Trim(l)
				target = "Imported/"+StripDir(original)
			EndIf
			Print "   = Importing: "+original+" as "+target
			outjcr.addentry original,target,"zlib"
		Next
	EndIf	
